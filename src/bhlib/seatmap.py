...
# 8×8 bitmap font (digits + basic ASCII letters). Rendered at 2× → 16×16.
_FONT_8x8: dict[str, list[int]] = {
    ...
    # Uppercase A-Z (simple block font; 8×8, last row blank).
...
def _glyph_bits_for_char(char: str) -> list[int] | None:
    bits = _FONT_8x8.get(char)
    if bits is None and "a" <= char <= "z":
        bits = _FONT_8x8.get(char.upper())
    return bits
...
def _draw_char(
    ...
    bits = _glyph_bits_for_char(char)
    if not bits:
        return
    r, g, b = color
...
def render_seat_map_to_image_bytes(seats: list[dict]) -> bytes:
    ...
    for row_idx, row in enumerate(grids):
        ...
        for ch, bg, w in row:
            ...
            if _glyph_bits_for_char(ch) is not None:
